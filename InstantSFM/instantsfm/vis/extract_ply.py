...
#     # Load the checkpoint.
#     # (This assumes that the checkpoint was saved with a key "splats" as in your training code.)
#     checkpoint = torch.load(cfg.ckpt, map_location=runner.device)
#     for k in runner.splats.keys():
#         runner.splats[k].data = checkpoint["splats"][k]

#     # --------------------------------------------------------------------------
#     # Extract the parameters needed for export.
#     # (We use the same API as in your rasterize/export code snippet.)
#     # --------------------------------------------------------------------------
#     # Positions (N x 3)
#     means = runner.splats["means"].detach().cpu().numpy()
#     # Rotation quaternions (N x 4)
#     quats = runner.splats["quats"].detach().cpu().numpy()
#     # Scale (stored as log-scale, so we take exp)
#     scales = torch.exp(runner.splats["scales"]).detach().cpu().numpy()
#     # Opacity (apply sigmoid)
#     opacities = torch.sigmoid(runner.splats["opacities"]).detach().cpu().numpy()
#     opacities=opacities.reshape(opacities.shape[0],1)

#     # In your training code, if app_opt is disabled the color info is stored as two parts:
#     # "sh0" and "shN". For export we assume the first channel are the DC coefficients,
#     # and the remaining ones are the extra coefficients.
#     f_dc = runner.splats["sh0"].detach().cpu().numpy().reshape(means.shape[0], 3, 1)
#     f_rest = runner.splats["shN"].detach().cpu().numpy().reshape(means.shape[0], 3, -1)
#     # Use the quaternion as the rotation (i.e. rot_{0...3})
#     rotation = quats

#     # Save the data as a .ply file.
#     save_ply(means, f_dc, f_rest, opacities, scales, rotation, cfg.export_ply_path)
